...
const helpContent = {
	shellType: {
		header: "Các loại pháo hoa",
		body: "Chọn（Random）” để có được trải nghiệm tốt nhất！",
	},
	shellSize: {
	...
	longExposure: {
		...
		body: "Có thể giữ lại tia lửa do pháo hoa để lại",
	},
};
...
function init() {
	...
	setOptionsForSelect(appNodes.quality, [
		{ label: "Chậm", value: QUALITY_LOW },
		{ label: "Bình thường", value: QUALITY_NORMAL },
		{ label: "Nhanh", value: QUALITY_HIGH },
	]);
...
if (IS_HEADER) {
	...
	setTimeout(() => {
		...
		Promise.all(promises).then(init, (reason) => {
			console.log("Tải tệp tài nguyên không thành công");
			init();
